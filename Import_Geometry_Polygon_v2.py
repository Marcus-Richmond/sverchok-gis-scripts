...
if gpkg_path and gpkg_layer:
    ...
    # create json dictionary of geodataframe, to extract EPSG code from
    gpd1_json = json.loads(gpd1.crs.to_json())
    
    # extract EPSG code, assign to output parameter, to show what the layer's original CRS is
    crs_old = [[gpd1_json['id']['code']]]
    
    # assign input 'crs_ReprojectTo' (the user defined EPSG code to reproject the layer to), to output parameter, to show what the layer's new CRS is
    crs_new = crs_ReprojectTo[0]
    
    if crs_ReprojectTo:
        gpd1 = gpd1.to_crs(epsg=crs_ReprojectTo)
    else:
